...
Integrasi sudah ada di `lessons.js` (recordPerformance). Pelajaran sekarang punya feedback loop → semakin "pintar" seiring waktu.

---

## DRY_RUN Mode

`DRY_RUN=true` di `.env` atau via `npm run dev` (sudah di-set otomatis).

**Prinsip:** Semua operasi on-chain yang mengirim transaksi real di-block — sebagai gantinya return object `{ dry_run: true, would_..., message: "DRY RUN — no transaction sent" }`.

### Operasi yang di-block saat DRY_RUN

| Lokasi | Fungsi | Yang di-skip |
|--------|--------|-------------|
| `tools/dlmm.js` | `addLiquidity()` | Tidak kirim TX, return detail posisi yang *akan* di-deploy |
| `tools/dlmm.js` | `claimFees()` | Tidak claim fee |
| `tools/dlmm.js` | `closePosition()` | Tidak close posisi |
| `tools/wallet.js` | `swapToken()` | Tidak swap token |

### Yang tetap jalan saat DRY_RUN

- **Screening** — pencarian pool tetap aktif
- **Balance check** di executor di-skip — tidak perlu punya SOL
- **Analisis & decision-making** — agent tetap bisa evaluate pool, hitung range, dll
- **HiveMind** — melaporkan status `dryRun: true` ke central
