...
async function telegramHandler(msg) {
  ...
  // ─── /health — external service connectivity check ───
  if (text === "/health") {
    await sendMessage("🔍 Running health checks...").catch(() => {});
    const checks = [
      { name: "RPC Primary", fn: async () => { const c = (await import("@solana/web3.js")).Connection; const conn = new c(process.env.RPC_URL); const slot = await conn.getSlot(); return `slot ${slot}`; }},
      { name: "RPC Fallback", fn: async () => { if (!process.env.RPC_URL_FALLBACK) return "not configured"; const c = (await import("@solana/web3.js")).Connection; const conn = new c(process.env.RPC_URL_FALLBACK); const slot = await conn.getSlot(); return `slot ${slot}`; }},
      ...
      { name: "HiveMind", fn: async () => { const r = await fetch(`${config.hiveMind.url}/health`, { signal: AbortSignal.timeout(8000) }); return `HTTP ${r.status}`; }},
    ];
    ...
    await sendMessage(`🏥 Health Check\n\n${results.join("\n")}`).catch(() => {});
    return;
  }
